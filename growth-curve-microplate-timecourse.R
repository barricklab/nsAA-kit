...
analyze_microplate <- function(dataFile, keyFile, output_base_name) {
  rawDataTbl <- read_excel(dataFile, col_names = FALSE, skip = 0) %>% slice(1:100)
  rawKeyTbl <- read_excel(keyFile)
  masterTbl <- parseIcontrolOutput(rawDataTbl, rawKeyTbl) %>% mutate(value = as.numeric(value))
  ...
  ggplot(expTbl, aes(x=time, y=meanValue, color=strain)) +
    ...
    scale_y_log10(breaks=c(0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.825)) +
    coord_cartesian(xlim = (c(0,24.5)), ylim=c(0.065,1.0)) +
  ...
  ggsave(paste0(output_base_name, ".all.pdf"))#, width=plot.width, height=plot.height)
