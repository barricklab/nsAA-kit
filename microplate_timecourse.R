...
analyze_microplate <- function(dataFile, keyFile, output_base_name) {
  ...
  netOD600Tbl <- expTbl %>% inner_join(blankTbl, by = "commonType") %>%
    separate(commonType, into = c("time", "AA", "replicate"), sep = "\\|") %>%
    select(-clone.y, -aaRS.y, -codon.y) %>%
    transmute(time = time, AA = AA, replicate = replicate, aaRS = aaRS.x, clone = clone.x,
              codon = codon.x, rawGFS = GFS.x, rawOD600 = OD600.x, blankOD600 = OD600.y) %>%
  ...
  p4 = ggplot(amberToTyrosineRatioGFSToRFSRatioTbl, aes(time, value, group=interaction(replicate, AA, clone), linetype=AA, color=clone)) + 
    geom_line() +
