...
for(this.aaRS in aaRS.list) {
 ...
 # background.noAA.wells = background.noAA.wells[1:3,]
  ...
  mega.table$pFRYC.noAA.ratio = mega.table$GFS.pFRYC.noAA.normalized / mega.table$RFS.pFRYC.noAA.normalized
  ...
  mega.table$pFRY.noAA.ratio = mega.table$GFS.pFRY.noAA.normalized / mega.table$RFS.pFRY.noAA.normalized
  ...
  ggsave(filename=file.path(paste(this.aaRS, "_decoding.efficiency.AA.pdf", sep="")))
 
 p = ggplot(mega.table, aes(x=time, y=mega.table$decoding.efficiency.noAA, color=clone, linetype=replicate))
 p + geom_line(shape=0, size=line_thickness) + coord_cartesian(ylim=c(-1,3))
 ggsave(filename=file.path(paste(this.aaRS, "_decoding.efficiency.noAA.pdf", sep="")))
 
 p = ggplot(mega.table, aes(x=time, y=mega.table$misincorporation.value, color=clone, linetype=replicate))
 p + geom_line(shape=0, size=line_thickness) + coord_cartesian(ylim=c(-1,3))
 ggsave(filename=file.path(paste(this.aaRS, "_misincorporation.value.noAA.pdf", sep="")))
}
